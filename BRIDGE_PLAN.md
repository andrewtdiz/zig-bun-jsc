...
- [x] `docs/bindings-map.md` (new): list every Zig file under `src/bun.js/bindings/` that is still imported; explicitly mark candidates for deletion. (2025-11-13 — table now lists only the five bindings the bridge consumes after dropping the stray `WTF.zig` export.)
- [x] For each unused binding file, delete the Zig + C++ pair and remove the re-export from `src/bun.js/jsc.zig`. (2025-11-14 — replaced the legacy `src/bun.js/bindings` tree with five lightweight stubs that match the bridge API.)
